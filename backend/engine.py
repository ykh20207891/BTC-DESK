...
log = logging.getLogger("btcdesk.engine")
...
BAR_MS = 15 * 60 * 1000
ROUTING_STALE_S = 10 * 60          # a ticket stuck in routing this long is abandoned
MAX_TICKETS_IN_MEMORY = 300
...
class Engine:
    def __init__(self, settings: Settings, store: Storage | None = None, rest: BybitREST | None = None):
        self.s = settings
        self.rest = rest or BybitREST(settings.api_key, settings.api_secret, settings.testnet)
        ...
        self.last_candle_update = 0.0
        self._ws_error_logged = 0.0
        ...
        self.seed = seed
        if not self.store.get("seed"):
            self.store.set("seed", seed)
        ...
        for t in self.store.load_tickets(MAX_TICKETS_IN_MEMORY):
            self.tickets[t["id"]] = t
        for t in self.store.load_open_tickets():          # never lose an open position to the memory cap
            self.tickets[t["id"]] = t
        ...
        # day state survives a restart, so a restart can never lift the drawdown guard
        self.day = datetime.now(timezone.utc).date()
        saved_day = self.store.get("day_state") or {}
        if saved_day.get("day") == self.day.isoformat():
            self.day_peak = float(saved_day.get("peak", self.equity))
            self.halted = bool(saved_day.get("halted", False))
        else:
            self.day_peak = self.equity
            self.halted = False
        self._last_eq_persist = 0.0
        self._last_live_equity = 0.0
    ...
    async def start(self):
        ...
        try:
            ...
            self.lot, self.min_qty, self.max_qty = inst["qty_step"], inst["min_qty"], inst["max_qty"]
            self.min_notional, self.tick = inst["min_notional"], inst["tick"]
            if isinstance(self.broker, LiveBroker):
                self.broker.qty_step, self.broker.tick = self.lot, self.tick
        except Exception as e:  # keep defaults, but say so
        ...
        # the last CONFIRMED bar; the forming bar's close is the first real trigger
        self._last_bar_ts = self.candles_15m[-2][0] if len(self.candles_15m) >= 2 else 0
        if isinstance(self.broker, LiveBroker):
    ...
    # ------------------------------------------------------------------ health
    def ws_age_s(self) -> float:
        return _now() - self.last_ws_msg if self.last_ws_msg else float("inf")

    def candle_age_s(self) -> float:
        return _now() - self.last_candle_update if self.last_candle_update else float("inf")

    def health(self) -> dict:
        ws_age, c_age = self.ws_age_s(), self.candle_age_s()
        ok = self.online and ws_age < 60 and c_age < 300
        return {
            "ok": ok, "online": self.online, "mode": self.s.mode, "paused": self.s.paused, "halted": self.halted,
            "ws_age_s": None if ws_age == float("inf") else round(ws_age, 1),
            "candle_age_s": None if c_age == float("inf") else round(c_age, 1),
            "reconnects": self.ws.reconnects if self.ws else 0,
            "open_positions": len(self.positions), "uptime_s": int(_now() - self.started), "equity": round(self.equity, 2),
        }
    ...
    def on_ws_status(self, ok: bool):
        ...
        self.online = ok
        if ok:
            self.last_ws_msg = _now()   # the connection itself is the first sign of life
        ...
        if ok and was is False and self.candles_15m:
            asyncio.get_event_loop().create_task(self._backfill())
        self.dirty = True

    async def _backfill(self):
        """After a reconnect, refill whatever the stream missed and fire any missed 15m close."""
        try:
            for row in await self.rest.klines(self.s.symbol, "1", 200):
                self._merge_kline(self.candles_1m, [row], raw=True)
            for row in await self.rest.klines(self.s.symbol, "15", 50):
                self._merge_kline(self.candles_15m, [row], raw=True)
            self.ticker.update(await self.rest.ticker(self.s.symbol))
            self.last_candle_update = _now()
            self.log_event("spotter", "SCAN", None, "backfilled the gap after reconnect", key="backfill")
        except Exception as e:
            self.log_event("core", "SYSTEM", None, f"backfill error · {e}", key="err", what="backfill", err=str(e))
        self._check_missed_close()

    def _check_missed_close(self):
        """The bar before the forming one is closed. If we never handled it, handle it now."""
        if len(self.candles_15m) >= 2 and self.candles_15m[-2][0] > self._last_bar_ts:
            self._last_bar_ts = self.candles_15m[-2][0]
            self._schedule_pipeline(reason="15m close")

    def on_ws(self, msg: dict):
        try:
            self._on_ws(msg)
        except Exception as e:  # never let one bad frame kill the feed
            if _now() - self._ws_error_logged > 60:
                self._ws_error_logged = _now()
                self.log_event("core", "SYSTEM", None, f"stream frame error · {e}", key="err", what="stream", err=str(e))

    def _on_ws(self, msg: dict):
        topic: str = msg["topic"]
        data = msg.get("data")
        self.last_ws_msg = _now()
    ...
    @staticmethod
    def _merge_kline(store: deque, data: list, raw: bool = False):
        for k in data:
            row = list(k) if raw else [int(k["start"]), float(k["open"]), float(k["high"]), float(k["low"]), float(k["close"]), float(k["volume"])]
            if store and store[-1][0] == row[0]:
                store[-1] = row
            elif not store or row[0] > store[-1][0]:
                store.append(row)
            else:  # an older bar (backfill): replace in place if we have it
                for i in range(len(store) - 1, max(-1, len(store) - 60), -1):
                    if store[i][0] == row[0]:
                        store[i] = row
                        break
    ...
    async def _broadcast_loop(self):
        while True:
            ...
            if not self.dirty or not self.clients:
                continue
            self.dirty = False
            payload = json.dumps({"type": "state", "state": self.snapshot()}, separators=(",", ":"))
            dead = []
    ...
    def recompute(self, full: bool):
        ...
        _mid, _up, _lo, z = zbands(closes15[-60:], 20, 2.0)
        ...
        st = self.stats()
        g["closer"]["gauge"] = round(st["win_rate"] * 100) if st["n"] else round(50 + self.dd_now() * -400)
        for d in g.values():
    ...
    def _persist_day(self):
        self.store.set("day_state", {"day": self.day.isoformat(), "peak": self.day_peak, "halted": self.halted})
    ...
    async def _mark_book(self):
        ...
        if not mark:
            return
        if isinstance(self.broker, LiveBroker):
            if _now() - self._last_live_equity >= 15:
                self._last_live_equity = _now()
                try:
                    self.equity = await self.broker.equity(list(self.positions.values()), mark)
                except Exception as e:  # keep the last known equity, but say so
                    self.log_event("core", "SYSTEM", None, f"wallet read failed · {e}", key="err", what="wallet", err=str(e))
        else:
            self.equity = await self.broker.equity(list(self.positions.values()), mark)
    ...
    def _sweep_stale(self):
        """Approval-only tickets expire at their horizon; a ticket stuck in routing is abandoned."""
        now = _now()
        for t in list(self.tickets.values()):
            if t["status"] == "on_deck" and now * 1000 >= t["horizon_ts"]:
                t.update({"status": "expired", "settled": now, "pnl": 0.0})
                self.store.save_ticket(t)
                self.log_event("taker", "PASS", None, f"ticket {t['id']} expired before approval", key="expired", tid=t["id"])
                self._reset_desks()
            elif t["status"] == "routing" and now - t["created"] > ROUTING_STALE_S and not (self._pipeline_task and not self._pipeline_task.done()):
                t.update({"status": "failed", "settled": now, "pnl": 0.0, "error": "routing stalled"})
                self.store.save_ticket(t)
                self.log_event("core", "SYSTEM", None, f"ticket {t['id']} abandoned · routing stalled", key="stalled", tid=t["id"])
                self._reset_desks()

    def _maintenance(self):
        if _now() - self._last_prune < 6 * 3600:
            return
        self._last_prune = _now()
        try:
            self.store.prune()
        ...
        settled = sorted((t for t in self.tickets.values() if t["status"] not in ("open", "on_deck", "routing")), key=lambda t: t["created"])
        for t in settled[:-MAX_TICKETS_IN_MEMORY]:
            self.tickets.pop(t["id"], None)

    async def _reconcile_live(self):
        """Compare our open tickets with the exchange before trading a single lot."""
        try:
            size, side = await self.broker.exchange_size()
        except Exception as e:
            self.log_event("core", "SYSTEM", None, f"reconcile failed · {e}", key="err", what="reconcile", err=str(e))
            return
        local = sum(p.qty for p in self.positions.values())
        if not self.positions and size > 0:
            self.log_event("core", "SYSTEM", None, f"exchange holds {size} BTC {side} with no desk ticket · left untouched, manage it manually", key="orphan_exchange", size=size, side=side)
        for tid, pos in list(self.positions.items()):
            if size <= 0 or side != pos.side:
                t = self.tickets[tid]
                t.update({"status": "settled", "exit": self.mark_price(), "pnl": 0.0, "settled": _now(), "reason": "reconcile", "stage": 5})
                self.positions.pop(tid)
                self.store.save_ticket(t)
                self.log_event("closer", "SETTLE", None, f"ticket {tid} closed on exchange while the desk was down · pnl unknown, booked as 0", key="orphan_ticket", tid=tid)
        if self.positions and abs(local - size) > self.lot:
            self.log_event("core", "SYSTEM", None, f"position size mismatch · desk {local} vs exchange {size} · closes use the exchange size", key="size_mismatch", local=local, exchange=size)
    ...
    async def settle(self, tid: str, reason: str):
        ...
        except Exception as e:  # keep the position and retry next tick
    ...
    def stats(self) -> dict:
        ...
        avg_edge = sum(t.get("edge", 0.0) for t in settled) / len(settled) if settled else 0.0
        peak, mdd = 0.0, 0.0
    ...
    def _schedule_pipeline(self, reason: str):
        if self._pipeline_task and not self._pipeline_task.done():
            self._pending_reason = reason        # run once the current ticket is routed
            return
        self._pipeline_task = asyncio.get_event_loop().create_task(self._run_pipeline(reason))

    async def _run_pipeline(self, reason: str):
        try:
            await self._pipeline(reason)
        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.exception("pipeline error")
            self.log_event("core", "SYSTEM", None, f"pipeline error · {e}", key="err", what="pipeline", err=str(e))
            self._reset_desks()
        finally:
            if self._pending_reason:
                reason2, self._pending_reason = self._pending_reason, None
                self._pipeline_task = asyncio.get_event_loop().create_task(self._run_pipeline(reason2))
    ...
    def size_ticket(self, direction: str, p_up: float, mark: float) -> dict:
        """Kelly size, then the exchange's lot/min/max/notional rules."""
        p_dir = p_up if direction == "UP" else 1 - p_up
        kz = calc_kelly(p_dir, self.equity, mark, self.s.risk, self.dd_notch(), self.lot)
        qty = round_step(kz["qty"], self.lot)
        if qty < self.min_qty or qty * mark < self.min_notional:
            qty = 0.0
        qty = min(qty, self.max_qty)
        kz["qty"] = qty
        kz["notional"] = round(qty * mark, 2)
        return kz
    ...
    async def _pipeline(self, reason: str):
        ...
        if self.halted:
            ...
            return
        if not self.online and reason != "boot scan":
            self.log_event("core", "HOLD", None, f"{reason} · stream offline · no ticket cut", key="hold_offline", reason=reason)
            return
        ...
        mark = self.mark_price()
        kz = self.size_ticket(direction, p_up, mark)
        notch = self.dd_notch()
    ...
    async def _execute(self, t: dict):
        ...
        try:
            ...
            return
        self.positions[tid] = pos
        t.update({"status": "open", "entry": round(pos.entry, 2), "qty": pos.qty, "sl": round(sl, 1), "tp": round(tp, 1),
                  "fee": round(pos.fee, 4), "position": pos.dict(), "stage": 5})
        self.store.save_ticket(t)
        self.log_event(
            "taker", "FILL", f"${pos.entry * pos.qty:,.0f}",
            f"{'bought' if direction == 'UP' else 'sold'} {pos.qty} BTC at ${pos.entry:,.1f} · sl ${sl:,.0f} · tp ${tp:,.0f} · {self.broker.name}",
            key="fill", dir=direction, qty=pos.qty, entry=f"${pos.entry:,.1f}", sl=f"${sl:,.0f}", tp=f"${tp:,.0f}", broker=self.broker.name,
        )
    ...
    def log_event(self, agent: str, action: str, amount: str | None, text: str, key: str | None = None, **p):
        ...
        if key:
            ...
            e["p"] = p
        self.log.append(e)
        log.info("%s %s %s %s", agent.upper(), action, amount or "", text)
        try:
            self.store.add_log(e)
        except Exception as ex:  # the ledger must never take the desk down
            log.warning("log persist failed: %s", ex)
        self.dirty = True
